namespace :db do
  [ :migrate, :rollback ].each do |n|
    namespace n do |_migration_task|

      original_task = Rake.application.current_scope.path
      puts "Running #{original_task} and writing sql"
      [:with_sql, :to_sql ].each do |t|
        ...
        task t => :environment do |sql_task|
          ...
          when "db:migrate"
            ...
            original_task = "db:migrate:up"
          when "db:rollback"
            filename = 'rollback.sql'
            original_task = "db:rollback:down"
          else
          ...
          # create or delete content of upgrade.sql
